...
- `/services` (semua member) — katalog on-demand (ephemeral): tiap kategori jasa (cuma yang `reviewable: true` di `ticketCategories.js` — Design, Dev, Cinematic; Complain & General ga relevan) tampil dengan deskripsi, harga (atau fallback ajakan buka tiket kalau belum diset admin), dan catatan tambahan kalau ada. Harga ditampilkan sebagai **spoiler** (`||teks||`) — ke-blur sampai di-klik.
